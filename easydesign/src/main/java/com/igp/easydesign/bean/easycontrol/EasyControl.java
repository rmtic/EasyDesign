...
public class EasyControl extends BaseEasyControl {
    ...
    public void bindEasyDesign(BaseEasyDesign easyDesign) {
        ...
        if(easyDesign != null){
            if (this.easyDesign.getSrcPs() != null && easyDesign.getSrcPs().length > 0) {
                this.srcPs = easyDesign.getSrcPs();
            }
            if (this.easyDesign.getSrcRect() != null) {
                this.srcRect = easyDesign.getSrcRect();
            }
            if (this.easyDesign.getDstPs() != null && easyDesign.getDstPs().length > 0) {
                this.dstPs = easyDesign.getDstPs();
            }
            if (this.easyDesign.getDstRect() != null) {
                this.dstRect = easyDesign.getDstRect();
            }
        }
    }
    ...
    public void drawLeftTopTips(@NonNull Canvas canvas) {
        ...
        canvas.drawText(String.format("Left[ %s dp ] Top [ %s dp ]",easyDesign.getDstRect().left,easyDesign.getDstRect().left),dstRect.left,dstRect.top - tipBoxTopDistance - tipBoxHeight / 4 - 80 ,paintTip);
        canvas.drawText(String.format("角度 [%s °],设计宽高[W:%sdp,H%sdp] [%s px,%s px],屏幕 %sdp,%spx",degree,width,height, ConvertUtils.dp2px(width),ConvertUtils.dp2px(height),ScreenUtils.getScreenWidth(),(ConvertUtils.dp2px(ScreenUtils.getScreenWidth()))),dstRect.left,dstRect.top - tipBoxTopDistance - tipBoxHeight / 4 ,paintTip);
        //绘制图片设计的信息
        if (easyDesign instanceof ImageEasyDesign) {
            ImageEasyDesign imageEasyDesign = ((ImageEasyDesign) easyDesign);
            canvas.drawText(String.format("图片类型[ %s ]",imageEasyDesign.getImageEasyDesignType()),dstRect.left,dstRect.top - tipBoxTopDistance - tipBoxHeight / 4 - 20 ,paintTip);
            if (easyDesign instanceof LocalImageEasyDesign || easyDesign instanceof RemoteImageEasyDesign) {
                canvas.drawText(String.format("源图宽高 [ %s , %s ]",imageEasyDesign.getOriginalWidthDp(),imageEasyDesign.getOriginalHeightDp()),dstRect.left,dstRect.top - tipBoxTopDistance - tipBoxHeight / 4 - 40 ,paintTip);
                canvas.drawText(String.format("模糊[%s]",imageEasyDesign.isBulr()),dstRect.left,dstRect.top - tipBoxTopDistance - tipBoxHeight / 4 - 60 ,paintTip);
            }
        }
    ...
    public void drawEasyIcons(Canvas canvas) {
        if (getBindEasyDesign() != null && getBindEasyDesign().isSelected()) {
            for (int i = 0; i < getBindEasyDesign().getDstPs().length; i+=2) {
                if(i== 0 && i+1 == 1){
                    //左上角图标
                    if (getEasyIconByIconType(EasyIconType.LEFT_TOP) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.LEFT_TOP).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.LEFT_TOP).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.LEFT_TOP).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.LEFT_TOP).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 2 && i+1 == 3){
                    //上中角图标
                    if (getEasyIconByIconType(EasyIconType.MIDDLE_TOP) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.MIDDLE_TOP).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.MIDDLE_TOP).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.MIDDLE_TOP).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.MIDDLE_TOP).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 4 && i+1 == 5){
                    //右上角图标
                    if (getEasyIconByIconType(EasyIconType.RIGHT_TOP) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.RIGHT_TOP).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.RIGHT_TOP).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.RIGHT_TOP).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.RIGHT_TOP).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 6 && i+1 == 7){
                    //右中角图标
                    if (getEasyIconByIconType(EasyIconType.RIGHT_MIDDLE) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.RIGHT_MIDDLE).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.RIGHT_MIDDLE).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.RIGHT_MIDDLE).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.RIGHT_MIDDLE).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 8 && i+1 == 9){
                    //右下角图标
                    if (getEasyIconByIconType(EasyIconType.RIGHT_BOTTOM) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.RIGHT_BOTTOM).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.RIGHT_BOTTOM).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.RIGHT_BOTTOM).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.RIGHT_BOTTOM).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 10 && i+1 == 11){
                    //下中角图标
                    if (getEasyIconByIconType(EasyIconType.MIDDLE_BOTTOM) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.MIDDLE_BOTTOM).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.MIDDLE_BOTTOM).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.MIDDLE_BOTTOM).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.MIDDLE_BOTTOM).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 12 && i+1 == 13){
                    //左下角图标
                    if (getEasyIconByIconType(EasyIconType.LEFT_BOTTOM) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.LEFT_BOTTOM).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.LEFT_BOTTOM).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.LEFT_BOTTOM).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.LEFT_BOTTOM).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 14 && i+1 == 15){
                    //左中角图标
                    if (getEasyIconByIconType(EasyIconType.LEFT_MIDDLE) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.LEFT_MIDDLE).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.LEFT_MIDDLE).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.LEFT_MIDDLE).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.LEFT_MIDDLE).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
                else if(i== 16 && i+1 == 17){
                    //中间角图标
                    if (getEasyIconByIconType(EasyIconType.CENTER) != null && dstPs != null) {
                        mPaintIcon.setAlpha( getEasyIconByIconType(EasyIconType.CENTER).getAlpha());
                        ...
                        canvas.drawBitmap(getEasyIconByIconType(EasyIconType.CENTER).getBitmap(), dstPs[i]-getEasyIconByIconType(EasyIconType.CENTER).getBitmap().getWidth()/2, dstPs[i+1]-getEasyIconByIconType(EasyIconType.CENTER).getBitmap().getHeight()/2, mPaintIcon);
                    }
                }
            }
